...
figure_2ori <- ggplot(data = table_1ori, 
  ...
  geom_point(shape = 1, colour = "black", fill = "white", stroke = 1,
             aes(size=Gene_length)) +
  xlim(1000, 3000) +
  labs(x = "Coding Region (bp)", y = "Exon (bp)", size = "Gene Length") +
...
#create a data set for box plot figure 5
dat <- melt(table_2ori_perc)
outlier <- dat$Species
dat <- cbind(outlier, dat)
dat <- dat  %>% group_by(variable) %>% 
  mutate(is_outlier=ifelse(is_outlier(value), value, as.numeric(NA)))
dat$outlier[which(is.na(dat$is_outlier))] <- as.numeric(NA)

fig5_ori <-
  ggplot(dat, aes(x=variable, y=value)) +
  geom_boxplot() + labs(x = NULL, y = "Fraction") +
  scale_x_discrete(labels = c("alpha Helix", "beta Sheet", "Coil")) +
  geom_text(aes(label=outlier),na.rm=TRUE,nudge_y =.01)

fig5_ori

#create a data set for box plot figure 5

dat <- melt(table_2primates_perc)
outlier <- dat$Species
dat <- cbind(outlier, dat)
dat <- dat  %>% group_by(variable) %>% 
  mutate(is_outlier=ifelse(is_outlier(value), value, as.numeric(NA)))
